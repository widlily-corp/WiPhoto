...
В версии **v4.1.0** добавлено многопоточное редактирование (Rayon), persistent-кэш метаданных в SQLite, ускоренный поиск дубликатов по BK-Tree, экономичный потоковый разбор RAW и промышленный XML-анализ XMP.
...
### 2. Локальный Кэш Метаданных в SQLite
Для ускорения сканирования медиафайлов внедрен persistent-кэш:
* Сведения о сканированных изображениях записываются в БД SQLite (`.wiphoto/library.db`).
* При повторных сканированиях сверяется дата модификации (`mtime`). Если файл не менялся, данные считываются напрямую из базы за микросекунды, минуя чтение EXIF и запуск ONNX-модели.

### 3. Поиск Дубликатов по BK-Tree
Для оптимизации выявления схожих изображений вместо полного перебора ($O(N^2)$) используется дерево Бурхарда-Келлера (**BK-Tree**):
* Все перцептивные хэши фотографий индексируются в дерево.
* Поиск дубликатов по расстоянию Хэмминга выполняется за субмиллисекундное время ($O(\log N)$).

### 4. Потоковое Чтение RAW в Чанках
Превью из «сырых» RAW-форматов (NEF, CR2, ARW и др.) извлекается без оверахэда RAM:
* Файл считывается последовательными блоками по 64 КБ для локализации маркеров JPEG (SOI/EOI).
* В физическую память загружаются только извлеченные байты сжатого JPEG-превью, снижая пиковую нагрузку с 100 МБ до менее чем 1 МБ на файл.

### 5. Многопоточное Редактирование (Rayon)
Обработка тяжелых пиксельных фильтров (яркость, контраст, тени, баланс белого и др.) параллелизована:
* Используются параллельные итераторы `Rayon` (`par_chunks_mut(4)`), задействующие все ядра процессора.
* Время применения изменений сокращено в 8-12 раз, обеспечивая отзывчивый real-time preview в редакторе.

### 6. Контекстная Панель Действий (Contextual Action Bar)
...
### 7. Гибридная Архитектура Тестирования (AAA Pattern)
Каждый тест написан в строгой структуре **Arrange-Act-Assert**:
* **Backend:** Юнит-тесты Rust (`cargo test`) покрывают кэширование БД, BK-Tree поиск, XMP-парсер, извлечение RAW и работу с файлами.
...
### 8. Автоматизированный CI/CD пайплайн (GitHub Actions)
В репозитории настроен рабочий процесс `.github/workflows/ci.yml`:
1. На каждый Push и Pull Request запускаются тесты JS и Rust.
2. При публикации тегов `v*` (например, `v4.1.0`) автоматически собираются дистрибутивы под Windows (`.msi`) и Linux (`.deb` / `.AppImage`), после чего формируется готовый черновик релиза в GitHub.
