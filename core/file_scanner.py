...
from core.analyzer import process_single_file, _worker_initializer, close_worker_exiftool
...
class Scanner(QObject):
    ...
    def start_scanning(self, root_folder: str, is_recursive: bool):
        ...
        try:
            ...
            # Инициализируем ExifTool daemon один раз в текущем потоке
            _worker_initializer()
            ...
            try:
                ...
                # Сначала закрываем ExifTool daemon — иначе при отмене futures
                # воркеры получают Broken pipe и спамят в лог
                close_worker_exiftool()
    ...
    def stop(self):
        ...
        # Закрываем daemon до отмены futures, чтобы не было Broken pipe
        close_worker_exiftool()
        if self.executor:
